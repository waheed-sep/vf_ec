...
# CONFIGURATION
# ==========================================
REPO_NAME = "libvncserver" # [UPDATED]
...
# [MAINTAINED] Robust file scanner handles BOTH Libtool and CMake hidden directories
...
# ==========================================
# [UPDATED] LIBVNCSERVER SPECIFIC
# ==========================================
def configure_libvncserver(cwd, coverage=False):
    flags = "-O0"
    libs = ""
    ...
    if os.path.exists(os.path.join(cwd, "CMakeLists.txt")):
        ...
        if not os.path.exists(os.path.join(cwd, "configure")):
            run_command("./autogen.sh", cwd)
        
        full_cmd = f'CFLAGS="{flags}" LDFLAGS="{flags}" LIBS="{libs}" ./configure --enable-static'
        return run_command(full_cmd, cwd)

def build_libvncserver(cwd):
    if os.path.exists(os.path.join(cwd, "cmake_build")):
        return run_command("cmake --build cmake_build -j$(nproc)", cwd)
    else:
        return run_command("make -j$(nproc)", cwd)

def get_libvncserver_tests(cwd):
    tests = []
    ...
    # Autotools logic
        ...
        tests.append({
            "name": "libvncserver-make-check",
            # [MAINTAINED] Forcing the build to ignore minor failures and harvest partial data
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_libvncserver(PROJECT_DIR, coverage=coverage): return None
    if not build_libvncserver(PROJECT_DIR): return None
    
    suite = get_libvncserver_tests(PROJECT_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # [MAINTAINED] Smart Gcov Generation
...
def prepare_for_energy_measurement():
    ...
    clean_repo(PROJECT_DIR)
    configure_libvncserver(PROJECT_DIR, coverage=False)
    build_libvncserver(PROJECT_DIR)
