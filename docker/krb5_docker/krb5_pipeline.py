...
# [NEW] krb5 source code lives in the 'src' subfolder
KRB5_SRC_DIR = os.path.join(PROJECT_DIR, "src")
...
# [MAINTAINED] Robust file scanner to handle Libtool's hidden directories
...
def process_commit(commit: str, coverage: bool = True):
    ...
    # [FIX] Force all operations to use the internal src/ directory
    if not configure_krb5(KRB5_SRC_DIR, coverage=coverage): return None
    if not build_krb5(KRB5_SRC_DIR): return None
    
    suite = get_krb5_tests(KRB5_SRC_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # [FIX] Run tests inside src/
        if not run_command(test.get('cmd'), KRB5_SRC_DIR):
            logging.warning(f"Test Build/Run Failed: {test.get('name')}")
            test['failed'] = True
            commit_results['tests'].append(test)
            continue
        
        # [MAINTAINED] Smart Gcov Generation tailored for Libtool folders
        gcda_files = get_gcda_files(KRB5_SRC_DIR)
        ...
        run_command(f"find . -name '*.gcov' -exec cp {{}} {test_gcov_dir}/ \\;", KRB5_SRC_DIR)

        # Clean up
        run_command("find . -name '*.gcda' -delete", KRB5_SRC_DIR)
        run_command("find . -name '*.gcov' -delete", KRB5_SRC_DIR)
...
def prepare_for_energy_measurement():
    ...
    # [FIX] Clean and rebuild inside src/
    run_command("make clean", KRB5_SRC_DIR)
    configure_krb5(KRB5_SRC_DIR, coverage=False)
    build_krb5(KRB5_SRC_DIR)
...
def measure_test(pkg_event, test, commit):
    ...
    for iteration in range(ITERATIONS):
        ...
        # [FIX] Energy loop now runs inside src/
        res = subprocess.run(perf_argv, cwd=KRB5_SRC_DIR, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, errors='replace')
