...
SRC_DIR = os.path.join(PROJECT_DIR, "src")  # Krb5 builds happen here
...
def get_git_diff_files(cwd, commit_hash):
    # For Krb5, we only care about changes inside /src
    ...
    for root, dirs, files in os.walk(cwd):
        for file in files:
            if file.endswith(".gcda"):
                ...
                # Krb5 source is often in the same dir as gcda
...
# ==========================================
# PROJECT-INDEPENDENT PATCH HELPER (ADDED)
# ==========================================
def patch_configure_empty_then(conf_path: str) -> bool:
    """
    Fix autoconf-generated shell syntax:
      if ...; then
      <blank>
      else
    by inserting a no-op ':' between then and else.

    Returns True if a change was made, else False.
    """
    try:
        if not os.path.exists(conf_path):
            logging.error(f"[patch_configure_empty_then] Missing: {conf_path}")
            return False
        ...
        if_then_re = re.compile(r'^\s*if\b.*;\s*then\s*$')
        ...
        for i, line in enumerate(lines):
            ...
            # Pattern: IF/THEN line, followed by blank line, followed by "else"
            if if_then_re.match(line):
                if i + 2 < len(lines) and blank_re.match(lines[i + 1]) and else_re.match(lines[i + 2]):
                    out.append("  :\n")
                    changed = True
        ...
        if changed:
            with open(conf_path, "w", encoding="utf-8") as f:
                f.writelines(out)
            logging.info(f"Patched empty-then blocks in: {conf_path}")

        return changed
    except Exception as e:
        logging.error(f"[patch_configure_empty_then] Exception: {e}")
        return False
...
def configure_krb5(cwd, coverage=False):
    logging.info("Regenerating configure script with autoreconf...")

    # 1. Force remove old configure script
    conf_path = os.path.join(cwd, "configure")
    if os.path.exists(conf_path):
        os.remove(conf_path)

    # 2. Regenerate
    if not run_command("autoreconf -fi", cwd):
        logging.error("Autoreconf failed.")
        return False

    # 3. Patch generated configure for empty-then blocks:
    #    if ...; then <blank> else ... fi  -> insert ":" between then and else
    patch_configure_empty_then(conf_path)
    ...
    # Use bash explicitly
    cmd = (
        f'CONFIG_SHELL=/bin/bash '
        f'CFLAGS="{flags}" LDFLAGS="{flags}" LIBS="{libs}" '
        f'/bin/bash ./configure --disable-rpath'
    )
...
def get_krb5_tests(cwd):
    tests = [{
        "name": "make-check",
        # Use -k (keep going) so we run as many tests as possible
        "cmd": "make -k check || true",
        "type": "make"
    }]
    return tests
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if os.path.exists(SRC_DIR):
        run_command("make distclean", SRC_DIR, ignore_errors=True)

    commit_results = {"hash": commit, "tests": []}

    # [IMPORTANT] All build steps happen in SRC_DIR
    if not configure_krb5(SRC_DIR, coverage=coverage):
        return None
    if not build_krb5(SRC_DIR):
        return None

    suite = get_krb5_tests(SRC_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        run_command("find . -name '*.gcda' -delete", SRC_DIR)

        # Run tests, ignore errors
        run_command(test.get('cmd'), SRC_DIR, ignore_errors=True)

        covered = get_covered_files(SRC_DIR)
        ...
        gcda_files_list = get_gcda_files(SRC_DIR)
        ...
        run_command("find . -name '*.gcda' -delete", SRC_DIR)
        run_command("find . -name '*.gcov' -delete", SRC_DIR)
...
def prepare_for_energy_measurement():
    ...
    run_command("make distclean", SRC_DIR, ignore_errors=True)
    configure_krb5(SRC_DIR, coverage=False)
    build_krb5(SRC_DIR)
