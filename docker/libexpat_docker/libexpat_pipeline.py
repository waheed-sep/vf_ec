...
def get_covered_files(cwd):
    covered = set()
    # Matches "anything ending in _la-" before the filename (Libtool mangling)
...
# LIBEXPAT SPECIFIC
# ==========================================
def get_expat_root(cwd):
    # Libexpat repository structure often puts the actual build system inside an 'expat' subdir
    if os.path.exists(os.path.join(cwd, "expat", "configure.ac")):
        return os.path.join(cwd, "expat")
    return cwd

def configure_libexpat(cwd, coverage=False):
    work_dir = get_expat_root(cwd)
    
    # 1. Generate configure if missing
    if not os.path.exists(os.path.join(work_dir, "configure")):
        logging.info("Generating configure script...")
        # Older versions use buildconf.sh
        if os.path.exists(os.path.join(work_dir, "buildconf.sh")):
            run_command("chmod +x buildconf.sh", work_dir)
            run_command("./buildconf.sh", work_dir, ignore_errors=True)
        
        # Fallback to autoreconf
        if not os.path.exists(os.path.join(work_dir, "configure")):
            run_command("autoreconf -fi", work_dir, ignore_errors=True)
    ...
    return run_command(full_cmd, work_dir)

def build_libexpat(cwd):
    work_dir = get_expat_root(cwd)
    return run_command("make -j$(nproc)", work_dir)

def get_libexpat_tests(cwd):
    work_dir = get_expat_root(cwd)
    # 'make check' is standard for expat
    tests = [
        {
            "name": "make-check",
            "cmd": "timeout 300s make check || true", 
            "type": "suite",
            "cwd_suffix": os.path.relpath(work_dir, cwd) if work_dir != cwd else ""
        }
    ]
    return tests
...
def process_commit(commit: str, coverage: bool = True):
    ...
    # Detect the actual build root (often PROJECT_DIR/expat)
    build_root = get_expat_root(PROJECT_DIR)

    if not configure_libexpat(PROJECT_DIR, coverage=coverage): return None
    if not build_libexpat(PROJECT_DIR): return None
    
    suite = get_libexpat_tests(PROJECT_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # Run test inside the build root
        run_command(test.get('cmd'), build_root, ignore_errors=True)
        
        # [GCOV Collection]
        gcda_files = get_gcda_files(build_root)
        covered_files_list = []
        ...
        # Use updated helper to get cleaner filenames
        test['covered_files'] = get_covered_files(build_root)
        ...
        # Move results
        run_command(f"find . -maxdepth 1 -name '*.gcov' -exec cp {{}} {test_gcov_dir}/ \\;", build_root)
        
        # Also check subdirectories for gcov files (often lib/ has them)
        run_command(f"find . -name '*.gcov' -exec cp {{}} {test_gcov_dir}/ \\;", build_root)
...
def prepare_for_energy_measurement():
    ...
    clean_repo(PROJECT_DIR)
    configure_libexpat(PROJECT_DIR, coverage=False)
    build_libexpat(PROJECT_DIR)
