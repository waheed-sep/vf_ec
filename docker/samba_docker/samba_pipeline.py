...
def configure_samba(cwd, coverage=False):
    ...
    try:
        if os.path.exists(os.path.join(cwd, "configure")):
            help_out = subprocess.run(["./configure", "--help"], cwd=cwd, capture_output=True, text=True).stdout
    except Exception as e:
        logging.warning(f"Could not read configure help, using default args. Error: {e}")

    if help_out:
        if "--without-fam" in help_out: config_args.append("--without-fam")
        if "--without-dmapi" in help_out: config_args.append("--without-dmapi")
        if "--without-tracker" in help_out: config_args.append("--without-tracker")
        if "vfs_snapper" in help_out: config_args.append("--with-shared-modules='!vfs_snapper'")
...
def get_samba_tests(cwd):
    tests = [{
        "name": "samba-quicktest",
        "cmd": "make -k quicktest || true",
        "type": "suite"
    }]
    return tests
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_samba(PROJECT_DIR, coverage=coverage): return None
    if not build_samba(PROJECT_DIR): return None
    
    suite = get_samba_tests(PROJECT_DIR)
