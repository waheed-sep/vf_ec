...
# [MAINTAINED] Robust file scanner to handle Libtool hidden directories
...
# ==========================================
# [UPDATED] LIBGD SPECIFIC
# ==========================================
def configure_libgd(cwd, coverage=False):
    # Libgd requires a bootstrap script to generate the configure files
    if not os.path.exists(os.path.join(cwd, "configure")):
        if not run_command("./bootstrap.sh", cwd):
            logging.error("Failed to run bootstrap.sh")
...
def get_libgd_tests(cwd):
    tests = [
        {
            "name": "libgd-make-check",
            # [FIX] Added '-k' to keep running after errors, and '|| true' to force a success signal 
            # so the pipeline harvests the partial coverage data.
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_libgd(PROJECT_DIR, coverage=coverage): return None
    if not build_libgd(PROJECT_DIR): return None
    
    suite = get_libgd_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    clean_repo(PROJECT_DIR)
    configure_libgd(PROJECT_DIR, coverage=False)
    build_libgd(PROJECT_DIR)
