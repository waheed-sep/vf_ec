...
# CONFIGURATION
# ==========================================
REPO_NAME = "FFmpeg"       # Target project to filter from the Gist CSV # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
...
SAMPLES_DIR = os.path.join(INPUT_DIR, "fate-samples") # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
...
def get_fate_tests(cwd):
    ...
    res = subprocess.run("make fate-list", cwd=cwd, shell=True, stdout=subprocess.PIPE, text=True)  # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
    tests = [l.strip() for l in res.stdout.split('\n') if l.strip().startswith("fate-")]    # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
    
    if TEST_LIMIT: 
        tests = tests[:TEST_LIMIT]
        
    return [{"name": t, "cmd": f"make {t} SAMPLES={SAMPLES_DIR} -j$(nproc)"} for t in tests]    # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
...
def run_phase_1_coverage(vuln, fix, master_csv_path, checkpoint_path):
    ...
    if cached_data.get("status") != "COMPLETE":
        ...
        run_command("./configure --disable-asm --disable-doc --extra-cflags='--coverage' --extra-ldflags='--coverage'", PROJECT_DIR)    # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
        run_command("make -j$(nproc)", PROJECT_DIR)
        
        suite = get_fate_tests(PROJECT_DIR)   # XXXXXXXXXXXXXXX PROJECT SPECIFIC XXXXXXXXXXXXXX
...
def detect_rapl():
    ...
    core = "power/energy-cores/" if "power/energy-cores/" in out else "power/energy-cores"  # Remove this line
    return pkg, core    # Remove 'core'
