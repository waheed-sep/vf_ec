...
# ==========================================
# [FIXED] LIBARCHIVE SPECIFIC
# ==========================================
def configure_libarchive(cwd, coverage=False):
    flags = "-O0"
    libs = ""
    ...
    # [FIX] Changed output directory to 'cmake_build' to avoid deleting Libarchive's internal 'build' source folder
    cmake_cmd = (
...
def build_libarchive(cwd):
    # [FIX] Point to new directory
    return run_command("cmake --build cmake_build -j$(nproc)", cwd)

def get_libarchive_tests(cwd):
    tests = []
    # [FIX] Point to new directory
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_libarchive(PROJECT_DIR, coverage=coverage): return None
    if not build_libarchive(PROJECT_DIR): return None
    
    suite = get_libarchive_tests(PROJECT_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # [MAINTAINED] AUTOMATED GCOV GENERATION
...
def prepare_for_energy_measurement():
    ...
    # [FIX] Delete the custom folder instead of the source folder
    run_command("rm -rf cmake_build", PROJECT_DIR)
    configure_libarchive(PROJECT_DIR, coverage=False)
    build_libarchive(PROJECT_DIR)
