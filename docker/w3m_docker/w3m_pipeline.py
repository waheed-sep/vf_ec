...
# [MAINTAINED] Robust file scanner to handle Autotools/Libtool hidden directories
...
# ==========================================
# [UPDATED] W3M SPECIFIC
# ==========================================
def configure_w3m(cwd, coverage=False):
    ...
    # w3m requires --with-gc to explicitly bind to the Boehm garbage collector
    config_args = [
        "./configure",
        "--with-gc", 
        "--with-ssl"
...
def get_w3m_tests(cwd):
    tests = [
        {
            "name": "w3m-render-readme",
            # [FIX] Launch the compiled browser to parse and render its own README file
            "cmd": "./w3m -dump README", 
            "type": "suite"
        }
    ]
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_w3m(PROJECT_DIR, coverage=coverage): return None
    if not build_w3m(PROJECT_DIR): return None
    
    suite = get_w3m_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    configure_w3m(PROJECT_DIR, coverage=False)
    build_w3m(PROJECT_DIR)
