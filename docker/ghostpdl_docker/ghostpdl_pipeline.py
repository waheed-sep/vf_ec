...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_ghostpdl(PROJECT_DIR, coverage=coverage): return None
    if not build_ghostpdl(PROJECT_DIR): return None
    
    suite = get_ghostpdl_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    configure_ghostpdl(PROJECT_DIR, coverage=False)
    build_ghostpdl(PROJECT_DIR)
