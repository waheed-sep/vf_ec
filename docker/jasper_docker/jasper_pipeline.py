...
def configure_jasper(cwd, coverage=False):
    ...
    if os.path.exists(os.path.join(cwd, "CMakeLists.txt")):
        ...
        cmake_cmd = (
            ...
            f'-DJAS_ENABLE_PROGRAMS=true '
            f'-DJAS_ENABLE_TESTING=true'
        )
        return run_command(cmake_cmd, cwd)
    else:
        # Old: Autotools (Pre-2016)
        if not os.path.exists(os.path.join(cwd, "configure")):
            run_command("autoreconf -fi", cwd)
        
        full_cmd = f'CFLAGS="{flags}" LDFLAGS="{flags}" LIBS="{libs}" ./configure'
        return run_command(full_cmd, cwd)

def build_jasper(cwd):
    # Detect build system
    if os.path.exists(os.path.join(cwd, "build")):
        # CMake uses its own build wrapper
        return run_command("cmake --build build -j$(nproc)", cwd)
    else:
        # Autotools standard make
        return run_command("make -j$(nproc)", cwd)

def get_jasper_tests(cwd):
    tests = []
    ...
    if os.path.exists(build_dir):
        ...
        # Older Autotools: Run standard make check
        tests.append({
            "name": "make-check",
            "cmd": "make check",
            "type": "make"
...
def process_commit(commit: str, coverage: bool = True) -> (dict | None):
    ...
    if not configure_jasper(PROJECT_DIR, coverage=coverage): return None
    if not build_jasper(PROJECT_DIR): return None
    
    suite = get_jasper_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    configure_jasper(PROJECT_DIR, coverage=False)
    build_jasper(PROJECT_DIR)
