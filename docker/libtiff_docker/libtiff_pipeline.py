...
# Storing .gcov text files hierarchically
...
# ==========================================
# [UPDATED] LIBTIFF SPECIFIC
# ==========================================
def configure_libtiff(cwd, coverage=False):
...
def build_libtiff(cwd):
    return run_command("make -j$(nproc)", cwd)

def get_libtiff_tests(cwd):
    tests = [
        {
            "name": "libtiff-make-check",
            "cmd": "make check",
            "type": "suite"
        }
    ]
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_libtiff(PROJECT_DIR, coverage=coverage): return None
    if not build_libtiff(PROJECT_DIR): return None
    
    suite = get_libtiff_tests(PROJECT_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # Copy the human-readable .gcov files
...
def prepare_for_energy_measurement():
    ...
    configure_libtiff(PROJECT_DIR, coverage=False)
    build_libtiff(PROJECT_DIR)
