...
# CONFIGURATION
# ==========================================
# [FIX] Changed to strictly lowercase to match Linux folder case-sensitivity
REPO_NAME = "freerdp" 
...
# FREERDP SPECIFIC
# ==========================================
def configure_freerdp(cwd, coverage=False):
    flags = "-O0"
    libs = ""
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_freerdp(PROJECT_DIR, coverage=coverage): return None
    if not build_freerdp(PROJECT_DIR): return None
    
    suite = get_freerdp_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    run_command("rm -rf build", PROJECT_DIR)
    configure_freerdp(PROJECT_DIR, coverage=False)
    build_freerdp(PROJECT_DIR)
