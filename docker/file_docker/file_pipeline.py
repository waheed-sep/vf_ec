...
# [FIX] Rewritten to capture raw gcda paths so we can handle Libtool .libs separation
...
# FILE SPECIFIC
# ==========================================
def configure_file(cwd, coverage=False):
...
def get_file_tests(cwd):
    tests = [
        {
            "name": "file-make-check",
            "cmd": "make check", 
            "type": "suite"
        }
    ]
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_file(PROJECT_DIR, coverage=coverage): return None
    if not build_file(PROJECT_DIR): return None
    
    suite = get_file_tests(PROJECT_DIR)
    ...
    for i, t in enumerate(suite):
        ...
        # [FIX] Smart Gcov Generation for Libtool
        ...
        for gcda in gcda_files:
            ...
            # If the file is in a secret .libs folder, tell gcov to run one folder up
            ...
            # Run gcov in the source folder, pointing at the hidden object folder
            run_command(f"gcov -p --object-directory {obj_dir} {source_name}", cwd=work_dir, ignore_errors=True)

        # Store a dummy list in JSON just to show that files were found
...
def prepare_for_energy_measurement():
    ...
    configure_file(PROJECT_DIR, coverage=False)
    build_file(PROJECT_DIR)
