...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_curl(PROJECT_DIR, coverage=coverage): return None
    if not build_curl(PROJECT_DIR): return None
    
    suite = get_curl_tests(PROJECT_DIR)
