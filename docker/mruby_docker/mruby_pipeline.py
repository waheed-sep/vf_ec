...
# ==========================================
# [FIXED] MRUBY SPECIFIC
# ==========================================
def configure_mruby(cwd, coverage=False):
    flags = "-O0"
    libs = ""
    ...
    # [FIX] Added 'test' target here so Rake compiles the test binary while flags are active
    full_cmd = f'CFLAGS="{flags}" LDFLAGS="{flags}" LIBS="{libs}" rake all test'
    return run_command(full_cmd, cwd)

def build_mruby(cwd):
    return True

def get_mruby_tests(cwd):
    # [FIX] Dynamically search for the test binary because older mruby commits 
    # put it in different folders (e.g., build/host/bin/) or rename it.
    test_bin = None
    build_dir = os.path.join(cwd, "build")
    
    # Walk the directory tree to find the compiled test binary
    if os.path.exists(build_dir):
        for root, dirs, files in os.walk(build_dir):
            for f in files:
                if f in ["mrbtest", "mruby-test"]:
                    candidate = os.path.join(root, f)
                    if os.access(candidate, os.X_OK): # Check if it is executable
                        test_bin = candidate
                        break
            if test_bin: break

    # Fallback just in case, though the search above should catch it
    if not test_bin:
        test_bin = os.path.join(build_dir, "test/bin/mrbtest")

    # Convert to relative path for safe execution
    rel_bin = os.path.relpath(test_bin, cwd)

    tests = [
        {
            "name": "mruby-direct-binary-test",
            "cmd": f"./{rel_bin}", 
            "type": "suite"
        }
    ]
...
def process_commit(commit: str, coverage: bool = True):
    ...
    if not configure_mruby(PROJECT_DIR, coverage=coverage): return None
    if not build_mruby(PROJECT_DIR): return None
    
    suite = get_mruby_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    clean_repo(PROJECT_DIR)
    configure_mruby(PROJECT_DIR, coverage=False)
    build_mruby(PROJECT_DIR)
