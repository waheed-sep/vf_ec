...
def get_covered_files(cwd):
    covered = set()
    # Libtool mangling regex (e.g. libsndfile_la-sndfile.c -> sndfile.c)
...
# LIBSNDFILE SPECIFIC
# ==========================================
def configure_libsndfile(cwd, coverage=False):
    if not os.path.exists(os.path.join(cwd, "configure")):
        ...
        if os.path.exists(os.path.join(cwd, "autogen.sh")):
             run_command("./autogen.sh", cwd, ignore_errors=True)
    ...
    config_args = [
        ...
        "--disable-shared",
        "--enable-static",
        "--disable-sqlite" # Keep build simple unless needed
    ]
...
def build_libsndfile(cwd):
    return run_command("make -j$(nproc)", cwd)

def get_libsndfile_tests(cwd):
    tests = [
        {
            "name": "libsndfile-check",
            # Timeout set to 300s (5 min) to catch infinite loops in audio parsers
...
def process_commit(commit: str, coverage: bool = True):
    ...
    # [FIX] Submodule init often needed for libsndfile dependencies
    run_command("git submodule update --init --recursive", PROJECT_DIR)

    commit_results = { "hash": commit, "tests": [] }

    if not configure_libsndfile(PROJECT_DIR, coverage=coverage): return None
    if not build_libsndfile(PROJECT_DIR): return None
    
    suite = get_libsndfile_tests(PROJECT_DIR)
...
def prepare_for_energy_measurement():
    ...
    clean_repo(PROJECT_DIR)
    configure_libsndfile(PROJECT_DIR, coverage=False)
    build_libsndfile(PROJECT_DIR)
